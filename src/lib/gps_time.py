#!/usr/bin/env python
"""
SOURCE: https://gist.github.com/davidbuzz/df4113f22dd0df5f3787b644ed6c5db1
...
@lru_cache(maxsize=1)
def leapseconds(tzfiles=("/usr/share/zoneinfo/right/UTC", "/usr/lib/zoneinfo/right/UTC"), use_fallback=True):
    ...
    for filename in tzfiles:
        ...
        if not use_fallback:
            ...
            warn("Using fallback leap seconds table", RuntimeWarning)
            return _fallback()

    with file:
        header = Struct(">4s c 15x 6i")  # see struct tzhead above
        (magic, version, _, _, leapcnt, timecnt, typecnt, charcnt) = header.unpack_from(file.read(header.size))
        if magic != "TZif".encode():
            raise ValueError("Wrong magic %r in tzfile: %s" % (magic, file.name))
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            [dict(utc=t.utc, tai=t.utc + t.dTAI_UTC, dTAI_UTC=t.dTAI_UTC.seconds) for t in leapseconds()],
            default=str,
            indent=4,
            sort_keys=True,
        )
